...
This repository intentionally contains only the Firefox extension source in `firefox-unpacked/`. The original Chrome unpack, CRX, ZIP, and XPI build artifacts are not tracked.
...
3. Select `firefox-unpacked/manifest.json`.
...
```sh
pnpm dlx web-ext lint --source-dir firefox-unpacked
pnpm dlx web-ext build --source-dir firefox-unpacked --artifacts-dir dist --overwrite-dest
```
...
- Minimum Firefox version is `142.0`, matching `firefox-unpacked/manifest.json`.
